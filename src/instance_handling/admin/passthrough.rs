...
mod post {
    ...
    pub async fn start_instance(Query(user): Query<AdminQuery>) -> impl IntoResponse {
    ...
    pub async fn refresh_instance(Query(user): Query<AdminQuery>) -> impl IntoResponse {
        match instance_api::Instance::refresh_user(&user.instance_name.unwrap_or_default()).await {
            Ok(started) => (StatusCode::OK, started.to_string()),
            Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, err.to_string()),
        }
        .into_response()
